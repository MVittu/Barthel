...
ui <- page_sidebar(
  title = tags$span(
    icon("heart-pulse"), " Barthel Index · Clinical Decision Support Dashboard"
  ),
  theme = bs_theme(
    bootswatch = "flatly",
    primary    = "#2C3E50",
  ...
  sidebar = sidebar(
    width = 280,
    h5(icon("user-doctor"), " Patient Profile", class = "text-primary"),
    hr(),
    selectInput("pathology", "Main Pathology",
    ...
    actionButton("predict_btn",
                 tagList(icon("rotate"), " Update Predictions"),
  ...
  layout_columns(
    col_widths = c(4, 4, 4),
    card(card_header(icon("utensils"),       " Feeding"),
         card_body(plotOutput("plot_feeding",    height = "290px"))),
    card(card_header(icon("person-walking"), " Ambulation"),
         card_body(plotOutput("plot_ambulation", height = "290px"))),
    card(card_header(icon("stairs"),         " Stairs"),
  ...
  card(
    card_header(class = "bg-primary text-white",
                icon("stethoscope"),
                " Biostatistical Synthesis & Clinical Recommendations"),
...
server <- function(input, output, session) {
  ...
  # allow_new_levels = TRUE marginalises over the new patient's random effect
  # by drawing it from the population prior (PatientID not seen in training).
  ...
  make_plot <- function(item_name, fill_hex) {
    d <- req(post_summary()) %>% filter(Item == item_name)
    ggplot(d, aes(x = .category, y = mean_p * 100)) +
      geom_col(fill = fill_hex, alpha = 0.85, width = 0.55) +
      geom_errorbar(aes(ymin = lo95 * 100, ymax = hi95 * 100),
                    width = 0.22, linewidth = 0.9, colour = "grey30") +
      geom_text(aes(label = sprintf("%.1f%%", mean_p * 100)),
                vjust = -0.7, size = 3.6, fontface = "bold") +
  ...
  output$plot_feeding    <- renderPlot(make_plot("Feeding",    "#2980B9"))
  output$plot_ambulation <- renderPlot(make_plot("Ambulation", "#27AE60"))
  output$plot_stairs     <- renderPlot(make_plot("Stairs",     "#8E44AD"))

  # Defined at server scope — recreating it inside renderUI on every render is wasteful.
  precision_badge <- function(width) {
    if (width > 0.30)
      ...
      tags$span(class = "badge bg-success text-white", "Good Precision")
  }
  ...
  output$synthesis <- renderUI({
    ...
    item_stats <- function(item) {
      ...
      if (nrow(r) == 0) return(list(m = 0, lo = 0, hi = 0, w = 0))
      list(m = r$mean_p, lo = r$lo95, hi = r$hi95, w = r$hi95 - r$lo95)
    }
    ...
    if (am$w > 0.30)
      notes <- c(notes, list(tags$li(
        ...
        sprintf(" Wide 95%% CrI (%.0f pp). Prognostic uncertainty is high.", am$w * 100),
        " Recommend re-evaluation after 2 weeks of rehabilitation."
      )))
    if (st$m < 0.25)
      notes <- c(notes, list(tags$li(
        tags$strong("Stairs:"),
        sprintf(" Low probability of independent stair use (%.1f%%).", st$m * 100),
        " Consider targeted stair-training programme or home adaptation assessment."
      )))
    if (f$m > 0.75)
      notes <- c(notes, list(tags$li(
        tags$strong("Feeding:"),
        sprintf(" High probability of independent feeding (%.1f%%).", f$m * 100),
        " Standard nutritional supervision should suffice."
      )))
    if (f$w > 0.30)
      notes <- c(notes, list(tags$li(
        tags$strong("Feeding:"),
        sprintf(" Considerable uncertainty in feeding outcome (CrI width %.0f pp).", f$w * 100),
    ...
    tagList(
      div(class = paste0("alert alert-", overall$cls, " mb-3"),
      ...
      tags$table(
        ...
        tags$thead(tags$tr(
          tags$th("Barthel Item"), tags$th("P(Independent)"),
          tags$th("95% Credible Interval"), tags$th("Precision")
        )),
        tags$tbody(lapply(
      ...
      tags$div(
        ...
        tags$p(class = "small text-muted mt-2",
               sprintf("Patient profile: %s · Age %d · Comorbidity: %s.",
                       isolate(input$pathology), isolate(input$age),
                       isolate(input$comorbidity)))
      )
    )
  })
}
